<?php
	// Esta clase se utiliza para registrar las acciones del usuario en la base de datos en la tabla bad_boot
	class Log {
		...
		public function find_all() {
			// Devolver todo 
			return $this->all = $this->db->query('SELECT * FROM bad_boot', PDO::FETCH_ASSOC);
		}
		...
		public function action($action = null, $additional_message = null) {
			...
			$name = $user->username ? $user->name . ' [' . $user->username . ']' : 'Robot Blacklist';
		...
		private function get_action($action = null, $additional_message = null) {
			...
			switch($action) {
				case 'view' : // Para visitas a la página
					$action = 'Directorio Visto: (' . page_name() . ')'; // Utilice la función page_name para especificar qué página ha visitado un usuario
					break;
